...
from Neo4jonline import Perception,SemanticMemory
from Neo4jonline import EpisodicMemory,WorkingMemory,PersonalSemantic
...
###########################################################
    ...
    tx=graph.begin()
    matcher = NodeMatcher(graph)
    nodesPersonae=list(matcher.match("Personae").where("_.PersonName ='%s'"%textName,"_.PersonReference='%s'"%textRefID))
    #textRefVerify= tx.evaluate("Match(n:Personae) WHERE n.PersonName='%s' RETURN n "%(textName))
    print(nodesPersonae)
    if not nodesPersonae:
        print("user not exist")
...
test=[]
threadlist=[]
for d in range(126,132):  
    #p.join()
    iu=d
    threadname=""+"memory"+str(iu)
    memory_thread=threading.Thread(target=memoryexecution, name=threadname, args=(df,d))
    memory_thread.start()
    threadlist.append(memory_thread)
    time.sleep(100)
    print("Processing")
    print(d)
#print(threadlist)
